...
public class ResCurrency extends OModel {
    ...
    OColumn rounding = new OColumn("Rounding Factor", OFloat.class)
    ...
    public static float compute(float price, ODataRow row){
        // TODO: 11/10/2017
        return price;
    }
    public static int _compute_decimal_places(ODataRow row){
        if(row == null || row.size() == 0){
            return 0;
        }
        if (0 < row.getFloat("rounding") && row.getFloat("rounding") < 1)
            return (int)Math.ceil(Math.log10(1/row.getFloat("rounding")));
        else
            return 0;
    }
    public static double round(ODataRow self, double amount){
        return amount;
    }
}
