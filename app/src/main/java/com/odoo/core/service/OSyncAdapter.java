...
public class OSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void syncData(OModel model, OUser user, ODomain domain_filter,
        ...
        Log.v(TAG, "Sync for (" + model.getModelName() + ") ==============================" +
                "Started at " + ODateUtils.getDate());
        Log.d(TAG, "syncData: ====================== createRelationRecords = "+String.valueOf(createRelationRecords));
        model.onSyncStarted();
        try {
    ...
    public HashMap<String, ISyncFinishListener> getmSyncFinishListeners() {
        return mSyncFinishListeners;
    }
}
